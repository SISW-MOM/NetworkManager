...
supplicant_interface_release (NMDeviceMacsec *self)
{
	...
	nm_clear_g_source (&priv->supplicant.ass_timeout_id);
...
static gboolean
supplicant_ass_timeout_cb (gpointer user_data)
{
	NMDeviceMacsec *self = NM_DEVICE_MACSEC (user_data);
	NMDeviceMacsecPrivate *priv = NM_DEVICE_MACSEC_GET_PRIVATE (self);
	...
	priv->supplicant.ass_timeout_id = 0;
                                     ...
                                     NMSupplicantInterfaceState state)
{
	...
	if (!priv->supplicant.ass_timeout_id)
		priv->supplicant.ass_timeout_id = g_timeout_add_seconds (15, supplicant_ass_timeout_cb, self);
}
                           ...
                           gpointer user_data)
{
	...
	nm_assert (!priv->supplicant.ass_timeout_id);
