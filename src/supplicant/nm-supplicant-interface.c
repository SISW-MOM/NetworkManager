                                  ...
                                  gpointer user_data)
{
	...
	_p2p_group_properties_changed (self, result);
	...
	if (nm_streq (signal_interface_name, NM_WPAS_DBUS_IFACE_INTERFACE_P2P_DEVICE)) {
		...
		if (nm_streq (signal_name, "GroupStarted")) {
			if (g_variant_is_of_type (parameters, G_VARIANT_TYPE ("(a{sv})"))) {
				...
				if (!g_variant_lookup (parameters, "group_object", "&o", &group_path))
					return;
				if (!g_variant_lookup (parameters, "interface_object", "&o", &iface_path))
					return;
